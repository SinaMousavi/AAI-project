...
def print_map(map):
    ...
    for i in range(len(map)):
        for j in range(len(map[0])):
            if map[i][j] == 'X':
                out = emoji.emojize(':white_large_square:', language='alias')
            elif map[i][j] == 'S':
                out = emoji.emojize(':car:', language='alias')
            elif map[i][j] == 'T':
                out = emoji.emojize(':fuelpump:', language='alias')
            elif map[i][j] == 'G':
                out = emoji.emojize(':checkered_flag:', language='alias')
            else:
                out = emoji.emojize(':black_large_square:', language='alias')
...
class Environment:
    X_COST = 5  # Costs for touching the wall. Problem with values < 3.
